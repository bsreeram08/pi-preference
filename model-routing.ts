...
function nativeRoutingGuidance(state: ModelRoutingState): string {
  const fixed = state.policy === "fixed" && state.fixed
    ...
    : "";
  return `Adaptive delegation routing: prefer first-party delegate_task for ordinary specialist work and workbench_agent_start when a persistent read-only agent must remain steerable or may ask the parent a question. Classify each lane independently from complexity, uncertainty, risk, breadth, and verification cost; role is only a prior. Balanced routes are light=${BALANCED_ROUTES.light.model}, standard=${BALANCED_ROUTES.standard.model}, heavy=${BALANCED_ROUTES.heavy.model}. A hard scout/recon lane can and should reach Sol; never use Spark for image/visual work. Before launch, show one compact line with role, model/thinking, reason, and read-only budget. Read-only limits are 8 turns/30 tools (light), 16/60 (standard), or 30/120 (heavy), with stop-and-synthesize guidance. Persistent mutation-capable agents are not enabled; use the existing single-writer delegate_task path under its lease. The external subagent tool is a temporary compatibility surface during migration and should not be chosen for new Workbench flows.${fixed}`;
}
