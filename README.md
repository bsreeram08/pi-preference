...
The main Workbench directory extension registers the repository-owned cmux bridge; it never edits or duplicates cmux's generated `cmux-session.ts` hook/feed bridge. The installer retains a dependency-free `cmux-workbench.ts` compatibility entry so existing Git installations and updater versions continue to validate the historical managed link without registering the bridge twice. Inside cmux, Pi uses a versioned metadata-only lifecycle contract with fixed categorical phase titles, states, progress, descriptions, and error codes. Prompts, tasks, outputs, summaries, raw errors, details, labels, and tool names are not accepted by that contract or forwarded into cmux command arguments. Explicit workspace/surface IDs prevent focus-dependent routing, command arguments are spawned without a shell, updates serialize, deduplication is bounded, and missing cmux state fails soft.

Each first-party AgentRunManager child launched inside cmux opens exactly one unfocused terminal surface in the caller's current workspace/pane. A trusted stdio bridge identifies the caller, creates `new-surface --type terminal ... --focus false`, records that exact surface, writes a private 0700 launcher, and sends only `bash <private-launcher-path>` to it. The launcher starts normal Pi TUI mode with the exact private session directory/ID, trusted child tools and control extension, exact model/tool/system-prompt loadout, no ambient extensions/skills/templates/context/themes, minimal environment, and inherited PTY stdio. Task/control delivery and bounded lifecycle/result events use a private authenticated Unix socket only. Native `ask_parent` exposes only its categorical `waiting`/`running` state through a private 0600 sidecar so cmux can title the tab without receiving the question or answer; there is no terminal typing of prompts and no screen scraping.

cmux command arguments and titles contain only fixed infrastructure paths/refs and an allowlisted categorical agent title; unknown or model-selected roles become `Specialist`. Prompt, task, output, question, model, credential, and raw error text never enters cmux commands, titles, status, logs, or notifications. `workbench_agent_focus` targets only the recorded surface. True normal settlement commits exact final text/session state, shuts down the child, returns the answer through AgentRunManager, and then closes only that surface. A direct-user abort leaves the interactive TUI open and waiting; manual tab closure fails the run. Dashboard pause/resume is disabled for interactive TUI runs rather than suspending only the bridge, and cancellation asks the child to abort/shut down before exact-surface cleanup and TERM/KILL escalation.
