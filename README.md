...
6. Completion requires independent, visible test evidence bound to the approved workflow task packet when present.
...
During a Workbench run, the footer shows the Supervisor and delegated agents as compact, collapsible phase cards. Use `Ctrl+Alt+A` to toggle dashboard focus, arrows to navigate, `Escape` to return to the editor, and `Enter` to open an agent overlay. The overlay streams output and tool calls and accepts queued steering messages. If a persistent agent is `waiting_for_parent`, submitting overlay input answers its exact pending question instead of steering. `P` pauses/resumes headless children (interactive Pi TUI children remain immediate and cannot be process-paused), selected-child `C` cancels only that child, and `Shift+C` first aborts the confirmed `/plan`, `/start-work`, or `/autopilot` run and then cancels its active children. `R` restarts eligible jobs, `F` shows files/tests, `Y` copies output, and `Escape` returns to Pi. Finished agents remain readable under `Finished (N)` until the next Workbench run.

### First-party persistent agent runtime

`delegate_task`, council work, and research now share one first-party AgentRunManager instead of owning separate launcher logic. Council Supervisor decisions are independent managed runs rather than a second subprocess authority. The manager uses shell-free process creation, strict LF-delimited RPC framing and command correlation, an exact pre-prompt tool-loadout handshake, a minimal environment, private persistent Pi sessions, integrity-checked atomic run records with unique attempt IDs, process-group cancellation with TERM/KILL escalation even when persistence fails, and fail-closed restart classification. Completion requires exact nonblank final text plus a settled, contained, regular private session checkpoint. Records live outside the project under `${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/workbench/agent-runs/v1/`; they retain task and output digests rather than raw task/output text, while the private Pi session necessarily contains the actual conversation.

The `workbench_agent_start`, `workbench_agent_message`, `workbench_agent_status`, `workbench_agent_answer`, `workbench_agent_cancel`, and `workbench_agent_focus` tools provide persistent interaction for read-only profiles, including Bash-capable specialists such as Codebase Explorer and Technical Reviewer. Inside cmux, each managed child is the actual normal Pi TUI in a new unfocused terminal tab in the caller's exact workspace/pane; focus it to chat or type directly. `ask_parent` uses the child's native TUI input there. Outside cmux, the existing headless RPC executor remains the compatibility runtime, including persisted parent question/answer delivery. Persistent mutation-capable agents remain deliberately deferred until lease/worktree recovery is implemented; use `delegate_task` for an approved single writer. AgentRunManager and its strict bridge protocol remain lifecycle/result authority in both modes.
...
| `/model-routing [status\|menu\|balanced\|economy\|quality\|codex\|grok\|fixed <model-or-alias>\|reset] [--default]` | Open a customize menu, or set session/durable child routing without changing Main Pi |
...
The extension itself is global at `~/.pi/agent/extensions/pi-workbench/`; intent, settings, workflow history, and research state live inside each project. Protected agent memory is project-keyed but stored under the Pi agent directory. Workflow `current.json` is the sole complete authority: validated saves serialize per project and atomically replace the plan projection before replacing `current.json`; plan and run files are non-authoritative projections and may be newer than `current.json` if the final authority write faults. Newly approved `/plan` and `/autopilot` plans carry `verificationMode: "packet"` plus a canonical terminal task-packet declaration bound by SHA-256 to the complete trimmed approved plan. The independent mode field prevents deletion of the packet field from silently downgrading verification. The packet contains bounded scope, non-goals, and at most 16 acceptance criteria only—never executable commands. States without `verificationMode` remain packetless version 1 legacy states and use the legacy verification marker.

Packet verification accepts one exact canonical criterion record bound to the packet ID and plan digest. `current.json` and packet run artifacts retain only concise structured evidence summaries, categorical protocol failures, and the SHA-256 digest of the exact verifier output; raw packet verifier output is not persisted. These records are structured verifier testimony, **not cryptographic or host-attested proof that commands ran**. Host-observed execution receipts are explicitly deferred. Malformed or unsafe authoritative state is reported as corruption rather than treated as absent. Confirmation-gated workflow and council commands compare an ephemeral in-memory authority snapshot immediately before launching work or changing persisted state; a mismatch launches nothing and requires a fresh run and confirmation. There is no automatic recovery, replay, resume, fresh-context handoff, sidecar transaction, or execution of packet-authored orchestration.

Workflow state writers, including `/plan`, and write-capable execution entrypoints first hold `.pi/pi-workbench/writer.lock`, keyed by the canonical resolved project/worktree root, then register that root under the global update coordination gate before launching children. Existing live, stale, ambiguous, or malformed project, gate, update, or writer artifacts block immediately and are never automatically removed or taken over. Independent roots remain concurrently writable when no update marker exists. Operator intervention must verify the owner before removing an abandoned artifact.
...
Adaptive model routing uses a compact three-effort interface. The shipped Codex family maps light/standard/heavy to `openai-codex/gpt-5.6-luna:low`, `openai-codex/gpt-5.6-terra:medium`, and `openai-codex/gpt-5.6-sol:high`. `/model-routing` opens a TUI customize menu. `/model-routing grok` or `use grok routing this session` switches children to the Grok 4.6 family for this session; `/model-routing grok --default` or the menu's **Save as project default** writes `modelRoutingFamily` for new sessions. Economy keeps Luna for light work, Spark for standard work, and Terra for heavy work in the Codex family, or Grok 4.6 low/low/medium in the Grok family; quality promotes one tier. Visual/image/UI/rendering work never uses Spark. Read-only children receive enforced stop-and-synthesize budgets of 8 turns/30 tools, 16/60, or 30/120; Workbench blocks excess tool calls and allows one final synthesis turn, while mutation-capable workers remain uncapped.

The opinionated `./install.sh --full` profile also makes Main Pi default to `openai-codex/gpt-5.6-sol` with high thinking. It transactionally backs up and replaces only the active provider/model/thinking defaults and theme while preserving unrelated settings. Child routing is independent: use `/model-routing` to customize family and policy, `--default` to persist them, or `pi --model xai/grok-4.6 --thinking high` if Main Pi itself should run Grok. Fixed aliases are `spark`, `luna`, `terra`, `sol`, and `grok`, and explicit fixed routes must use an available `openai-codex/<model>` or `xai/<model>[:low|medium|high]` registry entry (an omitted suffix becomes `:medium`). Exact standalone directives such as `use sol for everything`, `use grok for everything this session`, or `use grok routing this session` set a fixed or Grok-family route for delegated children. The state and receipts are persisted as TUI-only custom session entries, never added to model context, so new sessions return to the durable project policy. A fixed child route does not mutate Main Pi's current model. First-party `delegate_task` and `workbench_agent_start` receive the resolved route and compact receipt. The external `subagent` tool remains a temporary compatibility surface during migration and should not be selected for new Workbench flows.

### cmux task surfaces

The main Workbench directory extension registers the repository-owned cmux bridge; it never edits or duplicates cmux's generated `cmux-session.ts` hook/feed bridge. The installer retains a dependency-free `cmux-workbench.ts` compatibility entry so existing Git installations and updater versions continue to validate the historical managed link without registering the bridge twice. Inside cmux, Pi keeps lifecycle state in a versioned categorical contract while deriving a separate local work identity from the current project and task. Main-tab titles use `<project> · <task>` and workspace descriptions summarize the task; they remain stable through completion instead of becoming generic labels such as `Pi session · done`. Lifecycle completion, failure, and needs-attention are conveyed through notifications, status, and progress. Raw prompts, outputs, raw errors, details, tool names, URLs, control text, unsafe project basenames, common bare credential formats, high-entropy token-like values, and other secret-like project/task text are never forwarded into cmux command arguments. Explicit workspace/surface IDs prevent focus-dependent routing, command arguments are spawned without a shell, updates serialize, deduplication is bounded, and missing cmux state fails soft.

Each first-party AgentRunManager child launched inside cmux opens exactly one unfocused terminal surface in the caller's current workspace/pane. A trusted stdio bridge identifies the caller, creates `new-surface --type terminal ... --focus false`, records that exact surface, writes a private 0700 launcher, and sends only `bash <private-launcher-path>` to it. The launcher starts normal Pi TUI mode with the exact private session directory/ID, trusted child tools and control extension, exact model/tool/system-prompt loadout, no ambient extensions/skills/templates/context/themes, minimal environment, and inherited PTY stdio. Task/control delivery and bounded lifecycle/result events use a private authenticated Unix socket only. Native `ask_parent` exposes only its categorical `waiting`/`running` state through a private 0600 sidecar; entering `waiting` produces a needs-attention notification without replacing the task title or exposing the question or answer. There is no terminal typing of prompts and no screen scraping.

cmux command arguments and titles contain fixed infrastructure paths/refs plus a bounded, locally derived project/task identity. Secret-like input, URLs, code blocks, controls, prompt bodies, outputs, questions, model text, credentials, and raw errors are excluded. Every derived title and description is truncated by UTF-8 bytes within the bridge's 128-byte and 256-byte limits without splitting Unicode code points, so non-ASCII project/task names cannot fail launch framing. Agent role remains categorical (`Specialist` for unknown or model-selected roles) and appears only in the bounded task description. `workbench_agent_focus` targets only the recorded surface. True normal settlement commits exact final text/session state, shuts down the child, returns the answer through AgentRunManager, and then closes only that surface. A direct-user abort leaves the interactive TUI open and waiting; manual tab closure fails the run. Dashboard pause/resume is disabled for interactive TUI runs rather than suspending only the bridge, and cancellation asks the child to abort/shut down before exact-surface cleanup and TERM/KILL escalation.
