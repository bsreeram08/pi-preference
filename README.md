...
The `workbench_agent_start`, `workbench_agent_message`, `workbench_agent_status`, `workbench_agent_answer`, `workbench_agent_cancel`, and `workbench_agent_focus` tools provide persistent interaction for Bash-free read-only profiles. Inside cmux, each managed child is the actual normal Pi TUI in a new unfocused terminal tab in the caller's exact workspace/pane; focus it to chat or type directly. `ask_parent` uses the child's native TUI input there. Outside cmux, the existing headless RPC executor remains the compatibility runtime, including persisted parent question/answer delivery. Persistent mutation-capable agents and persistent Bash-capable public profiles remain deliberately deferred until lease/worktree recovery is implemented. Existing `delegate_task` remains the approved single-writer or shell-verification path. AgentRunManager and its strict bridge protocol remain lifecycle/result authority in both modes.
