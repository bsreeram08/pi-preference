...
During a Workbench run, the footer shows the Supervisor and delegated agents as compact, collapsible phase cards. Use `Ctrl+Alt+A` to toggle dashboard focus, arrows to navigate, `Escape` to return to the editor, and `Enter` to open an agent overlay. The overlay streams output and tool calls and accepts queued steering messages. If a persistent agent is `waiting_for_parent`, submitting overlay input answers its exact pending question instead of steering. `P` pauses/resumes, selected-child `C` cancels only that child, and `Shift+C` first aborts the confirmed `/plan`, `/start-work`, or `/autopilot` run and then cancels its active children. `R` restarts eligible jobs, `F` shows files/tests, `Y` copies output, and `Escape` returns to Pi. Finished agents remain readable under `Finished (N)` until the next Workbench run.

### First-party persistent agent runtime

`delegate_task`, council work, and research now share one first-party RPC child manager instead of owning separate launcher logic. Council Supervisor decisions are independent managed runs rather than a second subprocess authority. The manager uses shell-free process creation, strict LF-delimited RPC framing and command correlation, an exact pre-prompt tool-loadout handshake, a minimal environment, private persistent Pi sessions, integrity-checked atomic run records with unique attempt IDs, process-group cancellation with TERM/KILL escalation even when persistence fails, and fail-closed restart classification. Completion requires exact nonblank final text plus a settled, contained, regular private session checkpoint. Records live outside the project under `${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/workbench/agent-runs/v1/`; they retain task and output digests rather than raw task/output text, while the private Pi session necessarily contains the actual conversation.

The `workbench_agent_start`, `workbench_agent_message`, `workbench_agent_status`, `workbench_agent_answer`, `workbench_agent_cancel`, and `workbench_agent_focus` tools provide persistent interaction for Bash-free read-only profiles. A child may use first-party `ask_parent`; Workbench persists at most one question for the entire run, marks answer delivery before notifying the child, and rejects stale or duplicate answers. Persistent mutation-capable agents and persistent Bash-capable public profiles remain deliberately deferred until lease/worktree recovery is implemented. Existing `delegate_task` remains the approved single-writer or shell-verification path. When Pi runs inside cmux, every managed child also gets an unfocused browser tab in the current workspace/pane. The tab is a detachable local view only; AgentRunManager remains process and RPC authority.
...
Each first-party AgentRunManager child opens one unfocused browser surface—a tab—in the caller's current cmux pane. Workbench discovers the exact pane with `cmux identify`, creates the tab with `new-surface --type browser`, and renames it with only `<allowlisted agent category> · <categorical state>`; unknown or model-selected roles become `Specialist`. A private 0600 HTML projection under the run's private directory refreshes bounded assistant output and question state; output never appears in process arguments, cmux titles, status pills, logs, or notifications. `workbench_agent_focus` focuses the recorded surface explicitly. Closing or focusing the tab cannot start, steer, cancel, or otherwise own the child process.
