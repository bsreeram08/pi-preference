...
Context-loader changes must preserve strict handling of existing repository instructions while treating selected skills as optional. Cover complete skill contents, byte limits, source precedence, and explicit omission reasons. Use the focused command in [Child instructions and skills](docs/child-context.md), then the relevant child-runtime and workflow checks. Loading a design skill successfully is not evidence of improved visual output.
...
- Keep [context behavior](docs/child-context.md), [troubleshooting](docs/troubleshooting.md), and [testing steps](docs/testing-harness.md) aligned with changes to child launch, coordination, or verification. Label historical roadmap snapshots clearly rather than presenting resolved gaps as current behavior.
