...
- Added one optional unfocused browser tab per managed child in the caller's current cmux workspace/pane, backed by a private atomically refreshed bounded local view; cmux remains presentation-only and receives no prompt/output in command arguments or titles.
